...
if web=="Home":
    slt.image("2207288.webp",width=200)
    ...
    slt.subheader(":violet[Developed-by:]  Kavitha Thangavel")
if web=="About":
    ...
    with  col2:
        slt.image("PhonePe.webp",width=500)
...
if web=="GEO-VISUALIZATION":
        ...
        with tab1:
            ...
            if A=="Transaction":
                ...
                with col1:    
                    def agg_year1(year):
                            conn=mysql.connector.connect(host="localhost", user="root", password="kavi",database="phonepe")
        ...
        with tab2:
          ...
          if S=="Aggregated":
            ...
            if option_2=="Insurance":
                ...
                with col2:
                    fix=px.bar(dfQ_map_inc,x="District",y="total_amount",title=f"Transaction_amount-{Y_MI}",height=500,width=500)
                    slt.plotly_chart(fix)
    ...
    if Questions=="1.Top 20 aggregate insurance-transaction amount in state wise?":
       if slt.button("SUBMIT"):
            ...
            my_cursor.execute('''select State,sum(Transaction_amount)as total_amount from agg_inc
                              group by State 
                            order by sum(Transaction_amount) desc limit 20''')
            out=my_cursor.fetchall()
    ...
    if Questions=="2.Top most aggregate transaction-count in state wise?":
       if slt.button("SUBMIT"):
            ...
            my_cursor.execute('''select Transaction_type,sum(Transaction_count) as count from agg_trans
                            ...
                            order by sum(Transaction_count) desc limit 20''')
            out=my_cursor.fetchall()
